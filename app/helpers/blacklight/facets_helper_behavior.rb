...
module Blacklight::FacetsHelperBehavior
  ...
  def render_facet_image_item(facet_field, item)
    if facet_in_params?(facet_field, item.value )
      ...
      render_facet_image_value(facet_field, item)
    end
  end
  ...
  # @option options [Boolean] :suppress_link display the facet, but don't link to it
  # @return [String]
  def render_facet_image_value(facet_field, item, options ={})
    path = path_for_facet(facet_field, item)
  ...
  def facet_image_content field, item
    facet_display_image(field, item) + facet_image_overlay(field, item)
  end
